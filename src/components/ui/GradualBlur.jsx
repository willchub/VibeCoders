...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${keyCap}`]) v = config[`mobile${keyCap}`];
      else if (w <= 768 && config[`tablet${keyCap}`]) v = config[`tablet${keyCap}`];
      else if (w <= 1024 && config[`desktop${keyCap}`]) v = config[`desktop${keyCap}`];
      setValue(v);
    };
